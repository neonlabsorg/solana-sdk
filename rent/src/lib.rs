...
pub const DEFAULT_LAMPORTS_PER_BYTE_YEAR: u64 = 0; // rent removed (F3)

/// Default amount of time (in years) the balance has to include rent for the
/// account to be rent exempt.
pub const DEFAULT_EXEMPTION_THRESHOLD: f64 = 0.0; // rent removed (F3)

/// Default percentage of collected rent that is burned.
///
/// Valid values are in the range [0, 100]. The remaining percentage is
/// distributed to validators.
pub const DEFAULT_BURN_PERCENT: u8 = 0; // rent removed (F3)
...
impl Rent {
    ...
    pub fn minimum_balance(&self, _data_len: usize) -> u64 {
        // rent removed (F3): every account is rent-exempt at zero balance
        0
    }
    ...
    pub fn with_slots_per_epoch(slots_per_epoch: u64) -> Self {
        ...
        Self {
            lamports_per_byte_year: 0, // rent removed (F3)
            exemption_threshold,
            ..Self::default()
        }
    }
